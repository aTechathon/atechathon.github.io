	...
	V3JM.addJurgenMeierTab = function() {
		...
		tab.innerHTML =
			...
			'</p></a>';
		tabJurgenMeier.onclick = function() { JA.toggleDialogs( V3JM.JurgenMeier );V3JM.ifr.style.display=''; };
		...
		for ( var i = 0, len = V3JM.files.length; i < len; i++ ) {
			file = V3JM.files[ i ][ 1 ];
			fileList += '<a href=JavaScript:V3JM.updateIframe("' + i + '"); >' + file + '</a><br>';
		}
	...
	V3JM.init = function() {
		...
		var number = Math.floor( Math.random() * V3JM.files.length );
		V3JM.updateIframe( number );
	};

	V3JM.updateIframe = function( number ) {
		var file = V3JM.files[ number ][ 0 ];
		var fname = 'http://atechathon.github.io/algesurf/parametric-equations/equation-files/' + file + '/' + file + '.html';

		if ( !V3JM.ifr ) {
			V3JM.ifr = JA.container.appendChild( document.createElement( 'iframe' ) );
			V3JM.ifr.height = window.innerHeight;
			V3JM.ifr.width = window.innerWidth;
			V3JM.ifr.style.cssText = 'border-width: 0; position: absolute; ';
		}
		...
		V3JM.JurgenMeier.parentElement.style.display = 'none';
		...
		V3JM.ifr.onload = function() {
			...
			divMsg1.innerHTML = 'Source: <b><a href=' + V3JM.files[ number ][ 2 ] + ' target=_blank >' + V3JM.files[ number ][ 3 ] + '</a></b>';
			divMsg2.innerHTML = 'File: <b><a href=' + fname + ' target=_blank >' + V3JM.files[ number ][ 1 ] + '</a></b>';
