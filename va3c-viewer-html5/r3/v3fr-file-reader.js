	...
	V3FR.addFileReaderTab = function() {
		var tab = JA.menu.appendChild( document.createElement( 'div' ) );
		tab.title = 'Choose from a number of equations to display';
		tab.innerHTML =
			'<a href=# id=tabFileReader ><p class=button >' +
				'<i class="fa fa-file-image-o"></i> Jurgen Meier Gallery' +
			'</p></a>';
		tabFileReader.onclick = function() { JA.toggleDialogs( V3FR.FileReader );V3FR.ifr.style.display=''; };

		V3FR.FileReader = tab.appendChild( document.createElement( 'div' ) );
		V3FR.FileReader.style.cssText = 'cursor: auto; display: none; ';

		var fileList = '* particularly pretty<br>After \'Egg\' only * have coefficienta <br>';
		var file;
		for ( var i = 0, len = V3FR.files.length; i < len; i++ ) {
			file = V3FR.files[ i ][ 1 ];
			fileList += '<a href=JavaScript:V3FR.updateIframe("' + i + '"); >' + file + '</a><br>';
		}
		V3FR.FileReader.innerHTML =
			'<h3>Parametric Equations</h3>' +
			'<p>Sourced from:<br><b><a href="http://www.3d-meier.de/tut3/Seite0.html" target="_blank">Parametrische Flächen und Körper</a></b></p>' +
			'<div >' + fileList + '</div>' +
			'<p style=text-align:right; >' +
				'<a class=button href=JavaScript:JA.toggleTab(V3FR.FileReader); ); >Close</a> ' +
			'</p>' +
		'';
	};
	...
	V3FR.init = function() {
		...
		var number = Math.floor( Math.random() * V3FR.files.length );
		V3FR.updateIframe( number );
	};

	V3FR.updateIframe = function( number ) {
		var file = V3FR.files[ number ][ 0 ];
		var fname = 'http://jaanga.github.io/algesurf/parametric-equations/equation-files/' + file + '/' + file + '.html';

		if ( !V3FR.ifr ) {
			V3FR.ifr = JA.container.appendChild( document.createElement( 'iframe' ) );
			V3FR.ifr.height = window.innerHeight;
			V3FR.ifr.width = window.innerWidth;
			V3FR.ifr.style.cssText = 'border-width: 0; position: absolute; ';
		}
		...
		V3FR.FileReader.parentElement.style.display = 'none';
		V3CO.controlsTab.parentElement.style.display = 'none';
		JAGE.geometryTab.parentElement.style.display = 'none';

		V3FR.ifr.onload = function() {

			app = V3FR.ifr.contentWindow;
...
//			JAPR.setRandomGradient();
			...
			divMsg1.innerHTML = 'Source: <b><a href=' + V3FR.files[ number ][ 2 ] + ' target=_blank >' + V3FR.files[ number ][ 3 ] + '</a></b>';
			divMsg2.innerHTML = 'File: <b><a href=' + fname + ' target=_blank >' + V3FR.files[ number ][ 1 ] + '</a></b>';
	...
	V3FR.updateRenderer = function() {
		app.renderer.shadowMapEnabled = true;
		app.renderer.shadowMapSoft = true;
	};

	V3FR.updateAbout = function () {
		JA.about.style.height = '400px';
		...
		JA.about.innerHTML =
			...
			'</small><br><br>' +
			'<p style=text-align:right; >' +
				'<a class=button href=JavaScript:JA.toggleTab(JA.about); >Close</a> ' +
			'</p>' +
		'';
	};
